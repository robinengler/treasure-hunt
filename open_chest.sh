...
for key in $KEY_1 $KEY_2 $KEY_3; do
    ...
    if ! $(is_valid_key "$key"); then
        echo "$(random_exclamation) That key in lock $key_nb isn't working! Let's find the correct one, sail ho!"
        exit 1
    fi
done
...
echo "If you are doing this exercise for the exam, here's the code: $(get_key_code 1)$(get_key_code 2)$(get_key_code 3))"
exit 0
